...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        # print('sents',sents)
        extend_sents = [['{'+word+'}' for word in sent ] for sent in sents]
        # print('extend_sents',extend_sents)
        return [[[self.char2id.get(ch,self.char_unk) for ch in word] for word in sent] for sent in extend_sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_char_idx = self.words2charindices(sents)
        sents_t = pad_sents_char(sents_char_idx,self.char2id.get('<pad>'))
        sents_var = torch.tensor(sents_t,dtype=torch.long,device = device)

        ###这里需要转置-seq_length first
