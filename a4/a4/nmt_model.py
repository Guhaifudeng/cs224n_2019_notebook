...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     Dropout Layer:
        ###         https://pytorch.org/docs/stable/nn.html#torch.nn.Dropoutdropout
        self.encoder = nn.LSTM(input_size=embed_size,hidden_size=hidden_size,num_layers=1,bias=True,bidirectional=True,dropout=dropout_rate)#batch_first没有seq_first效率高
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_enc_hiddens,(last_hiddens,last_cells) =self.encoder(packed_X) #return (src_len,b,h*2),(1-layer*2-bi,hidden)
        enc_hiddens,_ = pad_packed_sequence(packed_enc_hiddens) #ret (src_len,b,h*2)
        enc_hiddens = enc_hiddens.permute(1,0,2) # (src_len,b,h*2)->(b,src_len,h*2)
        init_decoder_hidden = self.h_projection(torch.cat([last_hiddens[0],last_hiddens[1]],dim=1)) #
        init_decoder_cell = self.c_projection(torch.cat([last_cells[0],last_cells[1]],dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded) # (tat_len,b)->(tat_len,b,e)
        # print(Y.size())
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj,torch.unsqueeze(dec_hidden,2)) # (b,src_len,h)*(b,h,1)->(b,src_len,1)
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t,dim=1),enc_hiddens),dim=1) #(b,1,src_len)*(b,src_len,2*h) -> (b,1,2*h) -->by squeeze (b,2*h)
        v_t = self.combined_output_projection(torch.cat([a_t,dec_hidden],dim=1)) #(b,2h+h)->(b,h)
